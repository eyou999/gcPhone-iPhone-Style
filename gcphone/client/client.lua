...
RegisterKeyMapping('+openphone', 'Ouvrir son téléphone', 'keyboard', 'G')

local bind = true

RegisterCommand('+openphone', function()
  if bind and (GetGameTimer() - GUI.Time) > 1500 then
...
AddEventHandler('gcPhone:receiveMessage', function(message)
  ...
  if message.owner == 0 then
    ...
    if ShowNumberNotification == true then
      text = 'Vous avez reçu un message du ~g~'.. message.transmitter
      for _,contact in pairs(contacts) do
        if contact.number == message.transmitter then
          text = 'Vous avez reçu un message de ~g~'.. contact.display
          break
        end
      end
    end
